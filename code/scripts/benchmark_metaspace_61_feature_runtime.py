...
from osirim_occidata.catalog import discover_table_pairs, load_mapping  # noqa: E402
from osirim_occidata.features import (  # noqa: E402
    _build_column_text,
    _get_encoder,
...
from osirim_occidata.meta_features.classical import CLASSICAL_FEATURES, compute_classical_features  # noqa: E402
from osirim_occidata.meta_features.syntax import SYNTAX_FEATURES, compute_syntax_features  # noqa: E402


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Benchmark MetaSpace 61-feature construction on representative table pairs "
            "and extrapolate the total runtime over all pairs."
        )
    )
    p.add_argument("--dataset-root", type=Path, default=Path("valentine"))
    p.add_argument("--out-dir", type=Path, default=Path("outputs/exp_occidata/reports/meeting_baselines_vs_metamatch/metaspace_61_runtime_benchmark"))
...
def build_pair_61_features(pair, args: argparse.Namespace) -> pd.DataFrame:
...
def main() -> None:
    ...
    timing_path = out_dir / "representative_pair_timings_61features.csv"
    ...
    for i, row in selected.iterrows():
        ...
        t0 = time.perf_counter()
        feat_df = build_pair_61_features(pair, args)
    ...
    estimate_path = out_dir / "all_pair_runtime_estimate_61features.csv"
    ...
    by_rep.to_csv(out_dir / "runtime_extrapolation_by_representative_61features.csv", index=False)
    ...
    summary = {
        ...
        "outputs": {
            ...
            "by_representative": str(out_dir / "runtime_extrapolation_by_representative_61features.csv"),
        },
    }
    (out_dir / "runtime_estimate_summary_61features.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
