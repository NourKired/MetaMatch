...
COMPANION = OUT / "companion_site_csv"
...
def read_feature_build_seconds() -> float:
    p = OUT.parent / "metaspace_61_runtime_benchmark/runtime_estimate_summary_61features.json"
...
def read_recalibrated_rf_train_seconds() -> tuple[float, str]:
    one_fold = pd.read_csv(OUT / "metaspace_rf_one_fold_real_train_test_time.csv").iloc[0]
...
def build_table_metaspace_classifiers() -> None:
    src = pd.read_csv(OUT / "metaspace_classifiers_metrics_mean_std.csv")
    table = src.rename(
        columns={
            "method": "classifier",
    ...
    table.to_csv(COMPANION / "table_1_metaspace_classifiers_all_to_all.csv", index=False)
    table.to_csv(OUT / "table_1_metaspace_classifiers_all_to_all.csv", index=False)


def build_winrate_csvs() -> None:
    by_pair = pd.read_csv(OUT / "effectiveness_baselines_metaspace_by_pair.csv")
...
def build_f1_distribution_csv() -> None:
    ...
    df = pd.read_csv(OUT / "effectiveness_baselines_metaspace_by_pair.csv")
...
def build_runtime_distribution_csv() -> None:
    ...
    # Replace MetaSpace pair runtime by RF predict-only application time allocated by candidate rows.
    rf_summary = pd.read_csv(OUT / "metaspace_rf_predict_only_microbenchmark_summary.csv").iloc[0]
    rf_total_predict_sec = float(rf_summary["application_rf_predict_only_sec_all_folds"])
    row_scores = pd.read_parquet(OUT.parent / "metaspace_rf_61features_seed42/metaspace_rf_61features_seed42_row_scores.parquet")
    counts = row_scores.groupby("pair_id").size().reset_index(name="n_candidate_rows")
    counts["method"] = "MetaSpace"
    ...
    metaspace_runtime = counts[["method", "pair_id", "runtime_sec_pair", "runtime_min_pair", "n_candidate_rows"]]

    runtime = runtime[runtime["method"] != "MetaSpace"].copy()
    runtime["n_candidate_rows"] = np.nan
    out = pd.concat([metaspace_runtime, runtime], ignore_index=True, sort=False)
...
def build_efficiency_split_table() -> None:
    ...
    rf_predict_summary = pd.read_csv(OUT / "metaspace_rf_predict_only_microbenchmark_summary.csv").iloc[0]
    metaspace_application_sec = float(rf_predict_summary["application_rf_predict_only_sec_all_folds"])
    ...
    for method in PLOT_ORDER:
        ...
        if method == "MetaSpace":
            ...
            preliminary_computation = "61 meta-features construction + RF training"
            preliminary_note = rf_train_note
            application_sec = metaspace_application_sec
    ...
    corrected_summary = pd.read_csv(OUT / "metaspace_rf_predict_only_microbenchmark_summary.csv")
    ...
    corrected_summary.to_csv(OUT / "metaspace_rf_predict_only_microbenchmark_summary.csv", index=False)
    corrected_summary.to_csv(COMPANION / "table_2a_metaspace_rf_time_components.csv", index=False)


def main() -> None:
    build_table_metaspace_classifiers()
    build_winrate_csvs()
    build_f1_distribution_csv()
    build_runtime_distribution_csv()
    build_efficiency_split_table()
    print(f"Saved companion CSVs in: {COMPANION}")
