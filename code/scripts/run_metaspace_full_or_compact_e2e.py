...
from benchmark_metaspace_compact10_feature_runtime import (  # noqa: E402
    COMPACT_FEATURES,
    compute_compact_syntax,
    h0_entropy_combined,
)
from osirim_occidata.catalog import discover_table_pairs  # noqa: E402
from osirim_occidata.config import load_baseline_config  # noqa: E402
from osirim_occidata.features import build_full_metaspace  # noqa: E402
from osirim_occidata.io_utils import safe_write_table, write_json, write_jsonl  # noqa: E402
from osirim_occidata.meta_features.classical import compute_classical_features  # noqa: E402
from osirim_occidata.splits import make_repeated_splits_70_30, materialize_fold_frames  # noqa: E402
from osirim_occidata.catalog import load_mapping  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="End-to-end MetaSpace run: build embeddings/features, split, train RF, learn threshold on train, test."
    )
    p.add_argument("--config", choices=["full60", "compact10"], required=True)
...
def build_pair_compact_labeled(pair, args: argparse.Namespace) -> pd.DataFrame:
...
def build_compact_metaspace(pairs, args: argparse.Namespace) -> pd.DataFrame:
    if args.feature_workers <= 1:
        frames = [build_pair_compact_labeled(p, args) for p in tqdm(pairs, desc="compact meta-features")]
    else:
        frames = []
        with ThreadPoolExecutor(max_workers=args.feature_workers) as ex:
            futs = {ex.submit(build_pair_compact_labeled, p, args): p.pair_id for p in pairs}
            for fut in tqdm(as_completed(futs), total=len(futs), desc="compact meta-features"):
...
def evaluate_rf(metaspace_df: pd.DataFrame, fold_manifests: List[dict], features: List[str], args: argparse.Namespace) -> tuple[pd.DataFrame, pd.DataFrame, dict]:
    fold_rows = []
    pair_parts = []
    fit_total = 0.0
...
def main() -> None:
    ...
    if args.config == "full60":
        metaspace_df = build_full_metaspace(
            pairs,
        ...
        metaspace_df = build_compact_metaspace(pairs, args)
    feature_build_sec = time.perf_counter() - t_features

    metaspace_path = safe_write_table(metaspace_df, args.out_dir / "metaspace.parquet")
    features = feature_columns_for_config(metaspace_df, args.config)
    write_json(args.out_dir / "selected_features.json", {"config": args.config, "n_features": len(features), "features": features})

    t_split = time.perf_counter()
    folds = make_repeated_splits_70_30(metaspace_df, n_splits=args.folds, seed=args.seed, test_size=0.30)
    fold_manifests = materialize_fold_frames(metaspace_df, folds, out_dir=args.out_dir / "folds")
    split_sec = time.perf_counter() - t_split

    fold_df, pair_df, rf_timing = evaluate_rf(metaspace_df, fold_manifests, features, args)
    ...
    timing = {
        "config": args.config,
        "n_pairs": int(metaspace_df["pair_id"].nunique()),
        "n_rows": int(len(metaspace_df)),
        "n_features": int(len(features)),
        "metaspace_path": str(metaspace_path),
