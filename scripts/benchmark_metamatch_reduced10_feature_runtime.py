...
from metamatch.meta_features.syntax import (  # noqa: E402
    char_ngrams,
    common_suffix_len,
    cosine_counts,
    jaccard,
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=Path(
            ...
            "metamatch_reduced10_runtime_benchmark"
        ),
    )
...
def compute_reduced_syntax(a: str, b: str) -> Dict[str, float]:
...
def build_pair_reduced_features(pair, args: argparse.Namespace) -> pd.DataFrame:
    ...
    out["feature_build_sec_pair_reduced10"] = float(time.perf_counter() - t0)
    return out
...
def main() -> None:
    ...
    timing_path = out_dir / "representative_pair_timings_reduced10.csv"
    ...
    for _, row in selected.iterrows():
        ...
        feat_df = build_pair_reduced_features(pair, args)
        measured = float(feat_df["feature_build_sec_pair_reduced10"].iloc[0])
        payload = {
            "pair_id": pair_id,
            "dataset": row["dataset"],
    ...
    all_df["estimated_feature_build_sec_pair_reduced10"] = np.nan
    ...
    for idx, row in all_df.iterrows():
        ...
        all_df.at[idx, "estimated_feature_build_sec_pair_reduced10"] = (
            float(row["candidate_pairs"]) * float(nearest["sec_per_candidate"])
        )
    all_df.to_csv(out_dir / "all_pair_runtime_estimate_reduced10.csv", index=False)
    ...
    summary = {
        ...
        "estimated_median_feature_build_sec_pair_reduced10": float(all_df["estimated_feature_build_sec_pair_reduced10"].median()),
        "measured_min_sec": float(reps["feature_build_sec_pair_reduced10"].min()),
        "measured_max_sec": float(reps["feature_build_sec_pair_reduced10"].max()),
        "measured_mean_sec": float(reps["feature_build_sec_pair_reduced10"].mean()),
    }
    (out_dir / "runtime_estimate_summary_reduced10.json").write_text(json.dumps(summary, indent=2))
