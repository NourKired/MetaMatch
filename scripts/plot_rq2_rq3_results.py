...
LABELS = {
    ...
    "pearson085_rf_importance_k10": "Reduced k=10",
}
...
def main() -> None:
    ...
    save_bar(
        ...
        OUT / "fig_rq3_full_vs_reduced_f1.png",
    )

    reduced = df[df["config"].isin(["full60", "pearson085_rf_importance_k10"])].copy()
    reduced["method"] = reduced["config"].map(LABELS)
    reduced[
        [
            "method",
            "n_features",
    ...
    ].to_csv(OUT / "rq3_full_vs_reduced_for_paper.csv", index=False)
